...
class DualModelInference:
    ...
    def reload_driving_model(self):
        if os.path.exists(config.DRIVING_MODEL):
            try:
                self.driving_model.load_state_dict(torch.load(config.DRIVING_MODEL, map_location=self.device))
                self.driving_loaded = True
                logger.info(f"Successfully loaded Driving Model from {config.DRIVING_MODEL}")
            except Exception as e:
                logger.error(f"Failed loading Driving Model: {e}")
        else:
            logger.warning("No Driving Model found. Train the prototype first!")
            self.driving_loaded = False

    def reload_bin_model(self):
        if os.path.exists(config.GARBAGE_BIN_MODEL):
            try:
                self.bin_model.load_state_dict(torch.load(config.GARBAGE_BIN_MODEL, map_location=self.device))
                self.bin_loaded = True
                logger.info(f"Successfully loaded Bin Model from {config.GARBAGE_BIN_MODEL}")
            except Exception as e:
    ...
    def predict(self, frame):
        ...
        with torch.no_grad():
            ...
            # Dual Run: Driving prediction (Only run if no bin detected to save CPU if approaching)
